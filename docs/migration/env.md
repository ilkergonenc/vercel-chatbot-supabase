...
| `SUPABASE_STORAGE_SIGNED_URLS` | Optional. Set to `true` when the attachment bucket is private so chat requests generate signed URLs for OpenAI. Defaults to public URL mode. |
| `SUPABASE_STORAGE_SIGNED_URL_EXPIRES_IN` | Optional signed attachment URL expiry in seconds. Defaults to `3600`. |
| `BOTID_SECRET_KEY` | Optional. BotId checks and Next config wrapping are enabled only when this is set. |
...
| `SUPABASE_STORAGE_SIGNED_URLS` | Optional private-bucket mode for model-facing attachment URLs. |
| `SUPABASE_STORAGE_SIGNED_URL_EXPIRES_IN` | Optional signed URL expiry in seconds. |
...
- For Supabase Storage, create a bucket named `chat-attachments` or set `SUPABASE_STORAGE_BUCKET`.
- Public bucket mode requires `NEXT_PUBLIC_SUPABASE_URL` to be a hosted HTTPS project URL, not a local Supabase URL, because OpenAI downloads attachments directly from that URL.
- Private bucket mode requires `SUPABASE_STORAGE_SIGNED_URLS=true`; the chat API regenerates signed URLs from the stored object path before sending attachment parts to OpenAI.
...
- Existing message file parts are backward compatible when their `name` is the stored object path or their `url` is a Supabase Storage public or signed object URL.
